...
def create_keyboard(user, page_number=1):
    ...
    buttons = [types.InlineKeyboardButton(text=place, callback_data=place) for place in list(
        user.user_places)[page_interval:page_interval + 10]]
    ...
    try:
        ...
        if recieved_data in user.user_places:
            ...
            if 'address' in user.user_places[recieved_data]:
                reply_text += "Адрес: " + \
                    str(user.user_places[recieved_data]['address'])
                bot.send_message(message.chat.id, reply_text)
